...
- Out of scope for v1: moving a world to another network, world templates,
  per-world plugin sets, in-game economy integration (Vault currency, in-game
  shops), live migration of a world with players still inside it. Paid tiers
  (5.10) are not economy integration: entitlements are granted from outside
  the plugin and the plugin never sees a currency, a price or a transaction.
- Border enlargement, listed as out of scope up to v0.4, is now in scope in
  the one direction that is safe. See FR-3c.
...
CREATE TABLE player_world (
  ...
  hardcore      BOOLEAN NOT NULL DEFAULT false,      -- fixed at creation, never edited; see FR-1b
...
CREATE TABLE player_world_member (
  ...
  died_at    TIMESTAMPTZ,             -- hardcore worlds only; NULL means alive. See FR-5b
  PRIMARY KEY (world_id, uuid)
);
...
-- One purchased, one-time world upgrade (FR-44). Unlike every other child
-- table here, this one does NOT cascade from player_world: a player who
-- deletes an upgraded world must get the upgrade back, not lose what they
-- paid for, so the reference is ON DELETE SET NULL and world_id IS NULL is
-- what "not yet redeemed" means (FR-47).
CREATE TABLE world_upgrade (
  id          UUID PRIMARY KEY,
  owner_uuid  UUID NOT NULL,           -- who bought it; never changes, not even on FR-31 transfer
  world_id    UUID REFERENCES player_world(id) ON DELETE SET NULL,
  kind        TEXT NOT NULL CHECK (kind IN ('STORAGE', 'BORDER')),
  amount      BIGINT NOT NULL CHECK (amount > 0),  -- bytes for STORAGE, blocks of radius for BORDER
  reference   TEXT NOT NULL UNIQUE,    -- the grantor's transaction id; what makes a grant idempotent (FR-44)
  granted_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
  redeemed_at TIMESTAMPTZ             -- NULL together with world_id; see FR-45
);

-- "what has this player bought, and what is still unredeemed" is read on every
-- storage evaluation and every /world upgrades.
...
- FR-1b: A world may be created **hardcore**, with
  `/world create <name> [seed] hardcore`. Hardcore is a property of the world
  and is fixed at creation: it cannot be turned on for an existing world and
  cannot be turned off for a hardcore one. It therefore lives in a
  `player_world.hardcore` column rather than in `player_world.settings` — every
  setting in FR-9e and FR-9i is something the owner may change at any time, and
  putting this beside them would say the opposite of what it means. The reason
  it is fixed is the same as FR-3's for the border: it is the premise the world
  is played under. A world whose stakes can be added after the fact rewards
  everything already built under easier rules, and one whose stakes can be
  removed after a death undoes FR-5b for the player who has just paid it, which
  is the whole of what a hardcore world offers.
...
- FR-3: A world border of `border_radius` is applied to the overworld and the
  ...
  storage. Borders are enforced server-side. `border_radius` may afterwards be
  raised, but never lowered, under FR-3c; everything else in this requirement
  is unchanged by that, because a border is still re-asserted on every load
  from the database value rather than from `level.dat`.
...
- FR-3c: `border_radius` may be raised after creation, by an owner who holds
  the allowance for the larger radius (FR-43, FR-45). It may never be lowered.
  The asymmetry is not a simplification: enlarging exposes terrain that was
  always generatable from the world's seed, whereas shrinking strands whatever
  a player built in the ring it removes, on the wrong side of a barrier they
  cannot cross, with no way to recover it short of an admin enlarging the
  border again. A world border is also the only bound NFR-3 has on disk usage,
  so a raise is checked against `worlds.max-border-radius` and not merely
  against what the owner is entitled to.

  A raise writes `player_world.border_radius` and then asks the holding node to
  re-assert it (`APPLY_SETTINGS`, section 13). A world that is not loaded needs
  no message: FR-3 already re-asserts the border from the database on every
  load, which is the same path that makes a restored `level.dat` untrustworthy.
  This resolves the FR-9f interaction noted up to v0.4 — a world created
  private at radius 5000 and later made public can now be enlarged to match.
...
- FR-5a: Every dimension of a hardcore world (FR-1b) is generated with the
  hardcore flag set and its difficulty set to `HARD`, and both are re-asserted
  on every load — for the reason FR-3 and FR-9e give: both are persisted in
  `level.dat`, so the database value must win over whatever a folder restored
  from object storage happens to carry. A dimension materialised later on first
  transit (FR-2, FR-4) gets both at creation, so the nether and the end are
  never the soft way around a hardcore overworld.
- FR-5b: A death in a hardcore world by a member whose role is OWNER or BUILDER
  is permanent for that member. `player_world_member.died_at` is stamped with
  **database** time (CONTRIBUTING.md rule 5), the player is returned to the
  lobby, and every later attempt to enter that world is refused. The refusal is
  enforced on the proxy, in the same place as the ban check (FR-9d), because
  that is the one path every entry goes through and it answers whether or not
  the world is loaded.
  - Only that member is affected. The world stays `READY` and everyone else
    plays on, which is what makes this survivable for a world shared by
    several people: one player's mistake is not everybody's.
  - The owner is subject to it like anyone else, and a dead owner keeps every
    management command in section 6 — invite, kick, settings, transfer,
    archive, delete. They own a world they can no longer enter; the world
    still counts against their FR-1 cap until they delete or archive it, and
    deleting it is how they get the slot back.
  - VISITORs are exempt and respawn normally. A public world (FR-9c) grants
    VISITOR to anyone who walks in, and a stranger passing through must not be
    able to lock themselves out of a world permanently by dying once in it.
    A visitor promoted to BUILDER (FR-9) is subject from that moment on;
    a death they suffered as a visitor does not follow them.
  - Marking a member dead is idempotent and never overwrites an earlier
    `died_at` (CONTRIBUTING.md rule 7), so a retried control-plane delivery or
    a second death in the same tick cannot move the timestamp.
...
- FR-9e: Per-world settings the owner controls: PVP on or off, whether visitors
  ...
  folder carries.
- FR-9i: In addition to FR-9e, the owner controls the following vanilla
  gamerules per world: `keepInventory`, `fallDamage`, `fireDamage`,
  `freezeDamage`, `drowningDamage`, `advanceTime` (the day/night cycle),
  `advanceWeather`, `spawnPhantoms` (insomnia), `immediateRespawn`,
  `naturalHealthRegeneration`, `playersSleepingPercentage` (0-100),
  `maxEntityCramming`, `respawnRadius`, and `maxSnowAccumulationHeight`. These
  live in `player_world.settings` alongside FR-9e's settings, are applied to
  all three dimensions on load, and are re-asserted after a restore for the
  same reason as FR-9e: every one of these is persisted in `level.dat`, so the
  database value must win over whatever the restored folder carries.
  `randomTickSpeed` is deliberately excluded — it is a per-node performance
  knob, not a per-world one, and an owner cranking it up costs every other
  tenant on the node. Fire spread is not offered: the only rule the platform
  exposes for it, `FIRE_SPREAD_RADIUS_AROUND_PLAYER`, is a radius rather than
  the on/off toggle `doFireTick` used to be, which is a materially different
  control than the rest of this list.
...
The network sells two things, and they are different in kind rather than in
size. A **subscription** raises what a player is allowed across the board, for
as long as they keep paying. A **one-time upgrade** raises one world, once, and
is theirs from then on. Everything in this section follows from that difference.

- FR-41: An entitlement reaches the plugin in one of exactly two ways, and
  which one it is decides where it is stored.

  A subscription is **permissions**, granted by whatever the network already
  uses to grant ranks and revoked by the same thing when the subscription
  lapses. It needs no table here, and more importantly it must not have one:
  a copy of "is this player still subscribed" inside this plugin is a second
  answer to a question the permission backend is already authoritative about,
  and the copy is the one that goes stale when a chargeback happens at three in
  the morning.

  A one-time upgrade is a **`world_upgrade` row**, because a permission node
  cannot say which world it applies to. `gzmn.worlds.border.10000` is a
  statement about a player; "the border on *this* world is 10000 because they
  bought it" is a statement about a world, and there is nowhere in a permission
  string to put the world id. It is also the half that must outlive the grantor:
  a subscription that stops being paid for should stop applying, and a purchase
  that has been paid for once should not.

- FR-42: Three dials are for sale, and no others: the owner's storage
  allowance (§4, FR-30a), the number of worlds they may own (FR-1,
  `worlds.max-per-player`), and a world's border radius (FR-3, FR-3c). Each has
  a network default that applies to every player, and an entitlement can only
  raise a player above that default — never lower them below it. A tier is a
  floor, not a replacement: an operator who raises the network default has
  raised it for subscribers too, and does not have to re-issue anything.

- FR-43: Subscription tiers are read from permissions, highest held wins:

  | Node | Raises |
  | --- | --- |
  | `gzmn.worlds.storage.<size>` | storage allowance, e.g. `gzmn.worlds.storage.25gb` |
  | `gzmn.worlds.slots.<n>` | worlds the player may own, e.g. `gzmn.worlds.slots.5` |
  | `gzmn.worlds.border.<radius>` | the largest border they may raise a world to, in blocks |

  `gzmn.worlds.storage.<size>` already exists and is unchanged. The other two
  follow its rules exactly, including the reason `storage.quota-tiers` exists:
  a permission backend that can enumerate what a player holds (LuckPerms)
  honours any tier, and one that can only answer about a node it is handed
  (Velocity's own) can only be asked about the tiers named in configuration.
  `worlds.slot-tiers` and `worlds.border-tiers` are those lists.

  A subscription applies to every world the player owns, including worlds they
  created before subscribing and worlds transferred to them afterwards. It is a
  property of the player, so there is nothing to redeem and nothing to choose.

- FR-44: A one-time upgrade is granted by an administrator or, in practice, by
  the webstore acting as one:
  `/world admin upgrade grant <player> storage|border <amount> <reference>`.
  `reference` is the grantor's own transaction id and is `UNIQUE`, which is the
  whole of what makes the grant idempotent (CONTRIBUTING rule 7). A store that
  retries a delivery, or a staff member who runs the command twice because the
  first appeared to hang, grants one upgrade and is told so. A grant never
  needs the player online: it is a row keyed by UUID, and the player finds it
  waiting.

- FR-45: An upgrade is granted **unredeemed** and does nothing until its owner
  redeems it against one world they own, with `/world upgrades` or from the
  world's menu. Redemption sets `world_id` and `redeemed_at` in one statement
  conditional on both still being NULL, so two clients redeeming the same
  upgrade at once produce one redemption.

  What redemption does depends on the kind:

  - **`STORAGE`** adds `amount` bytes to the owner's allowance. The allowance
    is a per-player pool over every world they own (§4) and stays one — the
    world chosen is recorded, and shown, but the bytes are not fenced to it.
    Fencing them would mean a second quota with its own enforcement point on
    every snapshot commit, to solve a problem the pool does not have.
  - **`BORDER`** raises that world's `border_radius` by `amount` blocks under
    FR-3c, and is spent on it. This is the kind that needed a world in the
    first place.

- FR-46: Losing an entitlement never destroys anything. A lapsed subscription,
  or a network default lowered by an operator, can leave a player over their
  allowance in either dial; the effect is that they may not create or restore a
  world until they are back under it (which is what `worlds.max-per-player` and
  the storage quota already do), and nothing more. No world is archived, deleted
  or made unjoinable for being over an allowance, and a border already raised
  under FR-3c stays raised — FR-3c forbids lowering it whatever the reason.

  This is not leniency; it is the only behaviour that is safe. The alternative
  deletes a world because a card expired.

- FR-47: Deleting a world releases any upgrade bound to it back to unredeemed,
  rather than deleting it with the world (`ON DELETE SET NULL`, §4). This is the
  one child table of `player_world` that deliberately does not cascade. A player
  who deletes a world they had upgraded has thrown away the border, which is
  theirs to throw away, but not the purchase.
...
| --- | --- | --- | --- |
| `/world create <name> [seed] [hardcore]` | proxy | `gzmn.worlds.create` | Placement and lease first (FR-1a); `hardcore` is fixed at creation (FR-1b) |
...
| `/world border <radius> [world]` | proxy | owner | Raise only, never lower (FR-3c); bounded by FR-43 and `worlds.max-border-radius` |
| `/world upgrades [redeem <id> <world>]` | proxy | `gzmn.worlds.create` | Lists one-time upgrades and redeems one (FR-45) |
...
| `/world admin upgrade grant <player> <kind> <amount> <ref>` | proxy | `gzmn.worlds.admin` | FR-44; idempotent on `<ref>`, this is what a webstore calls |
| `/world admin upgrade revoke <ref>` | proxy | `gzmn.worlds.admin` | A refund or a mis-delivery; refuses once redeemed |
| `/world admin upgrade list <player>` | proxy | `gzmn.worlds.admin` | What they hold and against which worlds |
...
- `worlds.max-border-radius` (default 25000, the ceiling FR-3c checks a raise
  against regardless of entitlement — NFR-3 bounds disk usage by the border and
  nothing else does, so a mistyped permission tier must not be able to cost a
  node its disk)
- `worlds.slot-tiers` and `worlds.border-tiers` (the `gzmn.worlds.slots.<n>`
  and `gzmn.worlds.border.<radius>` tiers an operator has granted, FR-43). Read
  under exactly the condition `storage.quota-tiers` is: only where permissions
...
- OQ-21: FR-5b makes a hardcore death permanent and offers no way back. There
  is deliberately no revive command in this version, because a revive an admin
  can be asked for is one players will ask for, and a hardcore world whose
  deaths are negotiable is not one. But `died_at` is a nullable column and
  clearing it is all a revive would be, so the decision is reversible if
  operators find they need it for the case this omits: a death to a server
  fault rather than to the game. Confirm, and if a revive is wanted, say
  whether it is `gzmn.worlds.admin` only or something an owner may spend on
  their own world.
