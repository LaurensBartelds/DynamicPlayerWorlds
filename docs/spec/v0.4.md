...
- OQ-14: FR-39 requires "a table network staff can read" plus a captured chat
  ...
  marked handled?
- OQ-15: Section 6 places `/world leave` and `/world report` on the backend and
  everything else on the proxy, but a proxy plugin registering `/world` claims
  the entire namespace, so those two are unreachable unless the proxy forwards
  them explicitly. Confirm the proxy owns the root and forwards a known list of
  subcommands to the backend.
